...
- ✅ **Multi-step Signup**: 
  - Phone number verification with OTP (SMS via Twilio)
  - Email verification with OTP (Email via SMTP)
  - Password creation
- ✅ **OTP System**: 
...
- **OTP Verification**: Two-step verification for signup (phone + email)
  - **SMS OTP**: Sent via Twilio (configurable, falls back to console logging)
  - **Email OTP**: Sent via SMTP (Gmail, SendGrid, etc., configurable, falls back to console logging)
  - **Timeout Handling**: 15-20 second timeouts prevent hanging
...
│   │   ├── OTP.js            # OTP model (for verification with expiration)
...
#### OTP Verification
- `POST /api/auth/otp/generate/phone` - Generate and send phone OTP (SMS via Twilio)
- `POST /api/auth/otp/generate/email` - Generate and send email OTP (Email via SMTP)
